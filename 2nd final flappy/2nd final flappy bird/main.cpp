...
constexpr float PIPE_SPEED = 200.f;
...
int main() {
    ...
    if (!titleTex.loadFromFile("title.png")) return -1; // Load title image
    ...
    title.setPosition(WIDTH / 2.f - title.getLocalBounds().width / 2.f, HEIGHT / 8.f); // Position slightly lower than top
...
if (gameStarted && !gameOver && state == GAME) {
    ...
    // Spawn pipes
    if (pipeClock.getElapsedTime().asSeconds() > 1.6f) {
        pipeClock.restart();
        ...
        pipe.top.setScale(0.90f, -0.90f);  // Set the correct pipe scaling from the original code
